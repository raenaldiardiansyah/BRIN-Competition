...
const cta: Record<AIFeatureId, string> = { "collaboration-matching": "Mulai pencocokan", "innovation-profile": "Analisis profil & portfolio", "innovation-workspace": "Buka workspace + Copilot", "research-gap": "Temukan gap riset", "novelty-checker": "Cek indikasi kebaruan", "industry-matching": "Lihat preview Organization", "funding-recommendation": "Lihat preview Organization", commercialization: "Lihat preview Organization" };
...
export function AIHubPage() {
  ...
  return <div className="ai-page ai-hub-redesign">
    ...
    <FadeContent duration={420} translateY={12} blur={false} threshold={0.1}><section className="ai-section"><div className="ai-section__heading"><div><p>Advanced tools</p><h2>Direktori analisis lanjutan</h2></div><span>Prototype simulasi untuk keputusan berikutnya.</span></div><div className="ai-tool-directory">{advanced.map((item, index) => { const Icon = icons[index]; const organizationPreview = item.access.tiers.length === 1; return <div className={open === item.id ? "active" : ""} key={item.id}><button onClick={() => setOpen(item.id)}><Icon size={23} /><span><strong>{item.title}</strong><small>{item.description}</small></span><em>{organizationPreview ? "ORGANIZATION PREVIEW" : "PRO"} · PROTOTYPE</em><ArrowRight size={17} /></button>{open === item.id ? <section><p><b>Planned flow:</b> konteks → evidence → prioritas tindakan.</p><p><b>Limitation:</b> {item.limitationSummary}</p><a href={`${item.route}?source=ai-hub`}>{cta[item.id]} →</a></section> : null}</div>; })}</div></section></FadeContent>

    <AIHistorySection items={aiHistoryDummy} />
    <div className="ai-hub-limitation"><AILimitationNotice limitation={{ id: "ai-hub-local-data", title: "Prototype dengan data lokal", description: "Hasil memakai dummy data lokal; tidak terhubung ke AI/LLM, backend, API, atau database real-time. Evidence dan confidence hanya membantu penilaian dan tetap harus diverifikasi pengguna. Prototype tidak menjamin kompetensi, pendanaan, kecocokan mitra, atau keberhasilan proyek." }} /></div>
  </div>;
}
