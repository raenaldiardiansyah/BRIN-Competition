...
const Ferrofluid = ({
  ...
  const preparedColors = useMemo(() => prepColors(colorKey.split('|')), [colorKey]);
  ...
  useEffect(() => {
    ...
    const resolvedDpr = Math.min(dpr ?? (typeof window !== 'undefined' ? window.devicePixelRatio || 1 : 1), 1.5);
    ...
    const { arr, count, avg } = preparedColors;
    ...
    const renderFrame = (t: number) => {
      ...
      if (!paused && !prefersReducedMotion && inViewRef.current && document.visibilityState === 'visible') {
    ...
    const startRendering = () => {
      ...
      if (paused || !inViewRef.current || document.visibilityState !== 'visible') return;
    ...
    // Paint a deterministic first frame so reduced motion has a static visual.
    uniforms.iTime.value = 0;
    renderer.render({ scene: mesh });

    return () => {
      stopRendering();
      if (mouseInteraction && !prefersReducedMotion) window.removeEventListener('pointermove', onPointerMove);
